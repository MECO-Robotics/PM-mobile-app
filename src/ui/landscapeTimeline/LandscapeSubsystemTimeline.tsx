        ...
        {lanes.length === 0 && markerEvents.length === 0 ? (
          ...
          <View style={styles.contentRow}>
            <View style={[sidebarStyles.sidebar, { borderColor: colors.border }]}>
              <View
                style={[sidebarStyles.sidebarHeader, { borderColor: colors.border, backgroundColor: colors.surface }]}
              >
              ...
              {lanes.map((lane) => {
                const projectLabel = formatProjectLabel(lane);
            ...
            <ScrollView horizontal showsHorizontalScrollIndicator={false}>
              <View style={[chartStyles.chart, { backgroundColor: colors.canvas, width: chartWidth }]}>
                ...
                <View
                  style={[chartStyles.dayHeaderRow, { borderColor: colors.border, backgroundColor: colors.surface }]}
                >
                  ...
                  <View key={lane.id} style={[chartStyles.lane, { borderColor: colors.border, height: lane.height }]}>
                    ...
                    {lane.tasks.map(({ color, task, top }) => {
                      ...
                      return (
                        ...
                        >
                          <Text numberOfLines={1} style={chartStyles.taskBarText}>{task.title}</Text>
                        </Pressable>
                      );
                    })}
                  </View>
                ...
                {markerEvents.map(({ dayIndex, event }) => {
                  const markerLeft = dayIndex * DAY_WIDTH;

                  return (
                    <View
                      key={event.id}
                      pointerEvents="none"
                      style={[chartStyles.milestoneMarker, { left: markerLeft }]}
                    >
                      <View
                        style={[
